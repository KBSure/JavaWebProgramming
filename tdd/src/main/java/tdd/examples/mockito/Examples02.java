...
public class Examples02 {
    public static void main(String[] args) {
        ...
        CalculatorHistoryDao mock = mock(CalculatorHistoryDao.class);

//        when(mock.deleteById((long)10)).thenReturn(13);
        System.out.println(mock.deleteById((long)10));
        LinkedList mockedList = mock(LinkedList.class);

        // stubbing
        when(mockedList.get(0)).thenReturn("first");
        when(mockedList.get(1)).thenThrow(new RuntimeException());
        when(mockedList.get(2)).thenReturn(51);
...
//B 역할하는 mockedList가 Exception발생하면 어떻게 되는지 궁금할 때 B = mockedList;
